...
# =========================================================
# GLOBAL ALERT STATE (SAFE DEFAULT)
# =========================================================
alerts = []
...
# =========================================================
# 🎨 PREMIUM UI LAYER — STREAMLIT PRODUCT THEME
# =========================================================
st.markdown("""
<style>
...
.pill-yellow { background: rgba(250,204,21,0.15); color:#facc15; }
...
<div class="ui-card">
  ...
  <div style="display:flex; gap:12px; margin-top:12px">
    ...
    <span class="pill pill-yellow">Streaming</span>
    <span class="pill pill-orange">Output-Only</span>
    <span class="pill pill-red">Regulation-Ready</span>
  </div>
</div>
""", unsafe_allow_html=True)
...
# =========================================================
# 🧩 UI COMPONENT HELPERS (NO LOGIC)
# =========================================================

def ui_section(title, subtitle=None):
    st.markdown(f"""
...
def ui_card(title, value, subtitle=None, pill=None):
    st.markdown(f"""
    <div class="ui-card">
        <p>{title}</p>
        <div class="ui-metric">{value}</div>
        {f"<span class='pill {pill}'>{pill.replace('pill-','').upper()}</span>" if pill else ""}
        {f"<p>{subtitle}</p>" if subtitle else ""}
    </div>
    """, unsafe_allow_html=True)

def ui_alert(severity, title, description):
    color_map = {
        "low": "pill-green",
        "medium": "pill-yellow",
        "high": "pill-orange",
    ...
    pill = color_map.get(severity.lower(), "pill-yellow")
...
# =========================================================
# GLOBAL CLASS DISTRIBUTIONS (USED ACROSS TABS)
# =========================================================
current_freq = None
reference_freq = None
...
if PROBS_READY:
    ...
    if (
        current_df_tmp is not None and not current_df_tmp.empty and
        reference_df_tmp is not None and not reference_df_tmp.empty
    ):
...
with tab1:
    ...
    if demo_mode:
        st.info(
            "🧠 Judge Summary: The model shows early signs of entropy drift "
            "in low-confidence slices. Output stability remains intact. "
    ...
    if alerts:
        ...
        ui_alert(
            "low",
            "No Active Incidents",
            "System behavior within expected bounds."
        )

    ui_section("🚦 System Health", "Live system-wide monitoring")

    c1, c2, c3 = st.columns(3)
    with c1:
        ui_card("System Status", "Healthy", "No blocking actions", pill="pill-green")
    with c2:
        ui_card("Composite Drift Score", f"{(kl_score + psi_score)/2:.3f}", "Global risk")
    with c3:
        ui_card("Trend", "Stable →", "No acceleration detected")
...
with tab2:
    ...
    if reference_probs is not None:
        ...
        st.markdown("### 🚨 Drift Risk Indicators")
...
with tab6:
    ...
    if not PROBS_READY:
        ...
        fig = px.scatter(
            ...
            title="Confidence–Entropy Phase Space",
        )
...
with tab9:
    st.header("🧾 Self-Audit Report")
    st.info("Audit report generation is disabled in demo mode.")
...
score_df = pd.DataFrame({
    "Time": np.arange(len(global_score)),
    "Composite Drift Score": global_score
})

fig = px.line(
    score_df,
    x="Time",
    y="Composite Drift Score",
    title="Global Composite Drift Score Over Time"
)
